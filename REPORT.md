...
Seven deterministic corruption families are applied after image decoding and before the fixed evaluation transform: defocus blur, motion blur, Gaussian read noise, Poisson shot noise, JPEG compression, down–up resampling, and combined gamma/white-balance shift. Each has five parameters fixed before model results are examined. Blur sigma and motion-kernel length are defined at a 224-pixel reference short side and scaled to native image geometry, preventing SMIDS dimension variation from changing relative severity. The same sample receives paired corruption randomness across severity. A class-prior resampling condition doubles the SMIDS abnormal-class weight as a population-prevalence proxy; it is analyzed separately from the device-corruption average.
...
The shifts are simulated. Gaussian blur, ideal line-kernel motion, simple Poisson statistics, JPEG quality, bilinear resampling, and global gamma/channel gains do not capture a phone lens's modulation transfer function, demosaicing, denoising, sharpening, local tone mapping, or device-dependent color pipeline. White-balance/gamma is the least physically complete transformation. No severity is asserted to equal a clinical microscope or smartphone system.
